...
<head>
    ...
    <style>
        ...
        .media-container { height: 200px; width: 100%; background: #0b0b0b; display: flex; align-items: center; justify-content: center; }
        ...
        <div class="card">
            <div class="media-container">
                {% if item.file_type == 'image' %}
                    <img src="/file?path={{ item.current_path }}" loading="lazy" alt="{{ item.original_name }}">
                {% elif item.file_type == 'video' %}
                    <video src="/file?path={{ item.current_path }}" controls preload="none"></video>
                {% else %}
            ...
            <div class="info">
                ...
                {% if item.location_name %}
                    <div class="badge" style="background: #8b0000; color: #ffcccc;">📍 {{ item.location_name }}</div>
                {% endif %}
...
def index():
    ...
    # --- Apply Primary (Indexed) Filters First ---
    if source:
        query += " AND source = ?"
    ...
    # Date Range Logic: Pad the strings to cover the full day bounds
    if start_date:
    ...
    # Cap at 200 items to keep the browser memory lean
